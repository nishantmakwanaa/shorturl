...
function randomAlias() {
  const characters =
    "ae65r7t8yn9mu93145t32eyhrjkgmhn09i2gwhedcfvgbxc4v5b6n7ymuk089765rtdgvhbnm9iuhgvc";
  let result = "";
  ...
  try {
    ...
    if (!ali) {
      ...
      if (searchAli) {
        ...
        const data = new short_url({
          author: user.emailAddresses[0].emailAddress,
          destination_url: url,
          alias: random_alias,
        });
        await data.save();
        const setUserLimit = await User.findOneAndUpdate(
          { email: user.emailAddresses[0].emailAddress },
          { url_limit: getUserLimit.url_limit - 1 },
        );
